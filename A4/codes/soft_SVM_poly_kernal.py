    ...
    def Get_weight_zero(self, lagrange, X, y, weight):
        ...
        for i in range (num_item):
            if ( lagrange[i] > 0.0 ):
                ...
                K =  (1 + np.dot(X[i], X[i])) ** dim
                        
                #local_sum = y[i]- np.dot( weight, K)
                local_sum = y[i] - lagrange[i]*y[i]*K
                sigma += local_sum
        
        weight_zero = -1*(1/num_support_vector)*sigma
        
        return weight_zero
    
#=====================================#
def SVM_classify(X,y,Xt,yt,weight_zero, lagrange,):
    num_item, num_feature = X.shape
    numt_item, numt_feature = Xt.shape
    test_sum = 0.0
    error_sample = 0.0
    
    for i in range( num_item):
        for it in range( numt_item):
            if lagrange[it] > 0.0:
                K =  (1 + np.dot(Xt[it], X[i].T)) ** dim
                test = lagrange[it]*yt[it]* K
                test_sum += test
                
    
        class_test = test_sum + weight_zero
...
if __name__ == "__main__": 
    ...
    weight, weight_zero = soft_Poly_kernel_SVM_train().Get_weights( X_train, y_train)
    
    acc =SVM_classify(X_test, y_test, X_train, y_train,weight_zero, 
                 soft_Poly_kernel_SVM_train().Get_lagrange_multiplier(X_train, y_train))
    ...
    for train_index, test_index in kf:
        ...
        weightf, weight_zerof = soft_Poly_kernel_SVM_train().Get_weights( X_trainf, y_trainf)
        local_acc = SVM_classify(X_testf, y_testf,X_trainf, y_trainf, weight_zero, 
                                 soft_Poly_kernel_SVM_train().Get_lagrange_multiplier(X_trainf, y_trainf))
