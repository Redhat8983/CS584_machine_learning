...
#=========================================#
class soft_Gaussin_kernel_SVM_train(object):
    def __init__(self):
    ...
    def Gram_matrix(self, X):
        ...
        exponent = -np.sqrt(np.linalg.norm(X) ** 2 / (2 * sigma ** 2))
        K = np.exp(exponent)
...
if __name__ == "__main__": 
    ...
    weight, weight_zero = soft_Gaussin_kernel_SVM_train().Get_weights( X_train, y_train)
    ...
    for train_index, test_index in kf:
        ...
        weightf, weight_zerof = soft_Gaussin_kernel_SVM_train().Get_weights( X_trainf, y_trainf)
