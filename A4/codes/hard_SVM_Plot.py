...
class hard_SVM_train(object):
    ...
    def Get_lagrange_multiplier(self, X, y):
        ...
        G = cvxopt.matrix(np.diag(np.ones(num_item).T * -1))
        h = cvxopt.matrix(np.zeros(num_item).T)
...
if __name__ == "__main__": 
    ...
    weight, weight_zero = hard_SVM_train().Get_weights( X_train, y_train)
    
    acc = hard_SVM_classify(X_test, y_test, weight, weight_zero)
    
    print "============================================"
    print "Accuracy:", acc,"in", len(y), "test samples."
    ...
    for train_index, test_index in kf:
        ...
        weightf, weight_zerof = hard_SVM_train().Get_weights( X_trainf, y_trainf)
        local_acc = hard_SVM_classify(X_testf, y_testf, weightf, weight_zerof)
